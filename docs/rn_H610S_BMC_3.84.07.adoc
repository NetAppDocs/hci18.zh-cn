...
Element 版本 118 ， 12.0 和 12.2 支持此固件包。您可以直接带外升级H610S BMC固件，有关升级说明，请参见<<升级 H610S 节点上的 BMC 固件>>。
...
* 您已下载 BMC 更新软件包。下载BMC更新包 https://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab/download/62542/H610S_BMC_3.84["此处"^]。



NOTE: 此更新软件包包含 `rom.ima_enc`固件更新所需的文件和所有文件。
...
.步骤
. 使用Web浏览器登录到BMC控制台： `https://<BMC_IP>/#login`，其中是BMC的IP地址。 `<BMC_IP>`
. 选择*维护*。
. 选择*保留配置*。
. 执行以下操作：
+
...
. 在BMC Web UI中，在“*固件更新*”页面的“*选择固件映像*”部分下，选择“*选择文件*”。
. 浏览到 `rom.ima_enc`提取的软件包内容中的文件并将其选中。
. 在出现询问是否要继续的提示时，选择*Start Firmware Update*并选择*OK*。
. 出现提示时，选择*OK*，指示正在进行固件重置。
